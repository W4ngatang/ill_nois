...
class EnsembleGenerator(nn.Module):
    ...
    def forward(self, tanh_x, x, w, c, labels, model, mean=None, std=None):
        ...
        log_prob_dists = model(input_im) # n_models x n_ins x n_classes
        prob_dists = torch.exp(log_prob_dists)
        if self.targeted:
            losses = torch.sum(log_prob_dists * labels.unsqueeze(0).expand_as(log_prob_dists), dim=2) # n_models x n_ins
        else:
            losses = torch.sum(torch.log(1. - prob_dists + 1e-6) * labels.unsqueeze(0).expand_as(log_prob_dists), dim=2) # n_models x n_ins
        losses = losses.squeeze()
        class_loss = -torch.sum(model.weights.unsqueeze(-1).expand_as(losses) * losses, dim=0) # n_ins
        dist_loss = torch.sum(torch.pow(corrupt_im - x, 2).view(self.batch_size, -1), dim=1)
        ensemble_dist = torch.sum(prob_dists * model.weights.unsqueeze(-1).unsqueeze(-1).expand_as(prob_dists), dim=0).squeeze()
        return torch.sum(dist_loss + c * class_loss), dist_loss, \
                corrupt_im, torch.log(ensemble_dist)
